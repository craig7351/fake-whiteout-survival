...
/** GET /api/messages — 最新 120 則留言（含回覆，前端依 parentId 組成串） */
export const onRequestGet = async ({ env }: FnContext): Promise<Response> => {
  try {
    ...
    return json(results.map((r) => ({ id: r.id, name: r.name, text: r.text, at: r.created_at, parentId: r.parent_id ?? null })));
  } catch {
    return json({ error: 'db error' }, 500);
  }
};

/** POST /api/messages — 新增留言或回覆（限流 + 髒話過濾；parentId 有值＝回覆） */
export const onRequestPost = async ({ request, env }: FnContext): Promise<Response> => {
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as { name?: string; text?: string; deviceId?: string; parentId?: number };
    ...
    await env.DB.prepare('INSERT INTO messages (name, text, device_id, created_at, parent_id) VALUES (?, ?, ?, ?, ?)')
      .bind(name, text, device, Date.now(), parentId)
      .run();
    return json({ ok: true });
  } catch {
...
/** DELETE /api/messages — 版主刪除（需正確 key；連同其回覆一起刪） */
